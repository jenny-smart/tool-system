...
ALLOWED_ACTIONS = {"cetustek.login", "cetustek.download"}
...
def run_task(task: dict[str, str], *, service: Any, spreadsheet_id: str) -> int:
    ...
    try:
        ...
        lines.append(f"[{now_text()}] START {task['action']}")
        ...
        for raw_line in process.stdout:
            ...
            if time.monotonic() - last_upload >= 3:
                update_task(
                    row_number,
                    {"log": "\n".join(lines)[-45000:], "message": line[-500:] or "執行中"},
                    service=service,
        ...
        lines.append(f"[{now_text()}] {status.upper()} {message}")
        update_task(
            row_number,
            {
                "status": status,
                ...
                "message": message,
                "log": "\n".join(lines)[-45000:],
                "result_json": json.dumps({"exit_code": return_code}, ensure_ascii=False),
            },
            service=service,
            spreadsheet_id=spreadsheet_id,
        )
        ...
        lines.append(f"[{now_text()}] FAILED {type(exc).__name__}: {exc}")
        update_task(
            row_number,
            {
                "status": "failed",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tool System 鯨躍發票本機 Agent")
...
def main() -> int:
    ...
    print(f"Local Agent ready: {args.agent_id}", flush=True)
    while True:
        task = claim_next_task(agent_id=args.agent_id, service=service, spreadsheet_id=spreadsheet_id)
        if task:
            run_task(task, service=service, spreadsheet_id=spreadsheet_id)
            if args.once:
                return 0
        elif args.once:
            print("No queued task", flush=True)
            return 0
        else:
