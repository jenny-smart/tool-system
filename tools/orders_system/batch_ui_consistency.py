...
- 原「批次建單」固定逐列 single runner，不再誤走分組核心。
- 「批次建單優化」保留分組核心，但介面與原批次一致。
- 共用兩個自動篩選與可選的「查無班表自動補檸檬人」。
"""
...
from batch_booking_safety import run_process_web_single, run_process_web_optimized


def _bind_original_batch_to_single_runner() -> None:
    """ordersapp 先 from orders import run_process_web；直接改其 global binding。"""
        ...
        try:
            ...
            if filename.endswith("ordersapp.py") or name in ("ordersapp", "__main__"):
                if hasattr(module, "run_process_web"):
                    setattr(module, "run_process_web", run_process_web_single)
        except Exception:
            continue
...
def _parse_rows(batch_opt, raw: str) -> list[int]:
    ...
    if not raw:
        return []
    return batch_opt._parse_row_spec(raw)


def _region_credentials(region: str, fallback_email: str, fallback_password: str):
    account = ACCOUNTS.get(region) or {}
    email = str(account.get("email") or fallback_email or "").strip()
    password = str(account.get("password") or fallback_password or "").strip()
    ...
    try:
        ...
        for row_no in target_rows:
            ...
            if row is None:
                ...
                continue
            row_region = batch_opt.get_region_by_address(batch_opt._text(row.get("地址")), ACCOUNTS)
            if row_region != region:
                region_mismatch.append(row_no)
        if invalid:
            raise ValueError("以下列號不符合執行條件（必要資料不完整，或狀態/訂單編號不符）：" + "、".join(map(str, invalid)))
        if region_mismatch:
        ...
        ui_log("批次優化：保留同一人／地址／人數時數分組；每組完成立即回填。")
        result = run_process_web_optimized(
            ...
            logger=ui_log,
            allow_auto_lemon_shift=allow_auto_lemon,
            selected_rows=target_rows,
        ) or {}
        st.success(
            f"執行完成：成功 {int(result.get('success_count', 0) or 0)}，"
            f"失敗 {int(result.get('fail_count', 0) or 0)}。"
        )
