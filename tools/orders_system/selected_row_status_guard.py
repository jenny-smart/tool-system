...
def _load_worksheet_unique(sheet_name):
    ws, df = _ORIGINAL_LOAD_WORKSHEET(sheet_name); return ws, _dedupe_dataframe(df)
...
def _auto_filter_rows(batch_opt, sheet_name, mode, region=None):
    ...
    elif mode == "missing_order": mask = ~work.get("__o_col__", pd.Series("", index=work.index)).map(batch_opt._text).str.contains(r"(LC|TT|KK)\d+", regex=True, na=False)
...
def install_patch():
    ...
    orders.load_worksheet = _load_worksheet_unique; orders.update_sheet_rows = _update_sheet_rows_first_header
    orders.should_process_row = _should_process_row; orders.should_create_order = _should_create_order
    orders.ORDERS_VERSION = "v2026.09.05-4"; orders.ORDERS_UPDATED_AT = "2026-09-05"
    try:
