...
periods = st.multiselect(
    ...
    default=["08:30-12:30", "14:00-18:00"],
)

st.caption("第一版先支援一段期間一組 AM/PM 比例；之後可再增加多區間規則表。")
r1, r2 = st.columns(2)
with r1:
...
st.subheader("實際建立訂單")
st.info("為安全起見，第一版一次只執行一個『日期＋時段』。成功後可再選下一列；每張訂單前都會重新查即時未配班人數。")
...
confirm = st.checkbox("我確認目前是測試機，且這次要真的成立保留訂單")

if st.button("3. 確認建立保留單", type="primary", use_container_width=True, disabled=not confirm or execute_count <= 0):
    try:
