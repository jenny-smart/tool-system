...
def normalize_date_sort_value(value: Any) -> Any:
    ...
    try:
        return datetime.strptime(text.split(" ")[0], "%Y/%m/%d").timestamp()
    except Exception:
        return 999999999999
...
def sort_order_values(values: list[list[Any]]) -> list[list[Any]]:
    ...
    body.sort(key=lambda r: (normalize_date_sort_value(r[7] if len(r)>7 else ""), str(r[12] if len(r)>12 else "")))
    return [header] + body


def run_orders_for_area(cfg: dict[str, Any], area: str, date_key: str) -> None:
    ...
    values = ensure_rectangular(sort_order_values(read_file_values(drive, sheets, file)))
    if not values:
        raise RuntimeError(f"訂單資料沒有內容：{file_base}")
    clear_and_write_values(sheets, target_spreadsheet_id, f"{TARGET_SHEET_NAME}!A:ZZ", values)
    log(f"完成：{file.get('name')} → {TARGET_SHEET_NAME}!A:ZZ / rows={len(values)}")
...
def main(date_key: str | None = None, area: str | None = None, system_name: str = "外場日排程系統") -> None:
    ...
    for a in ([area] if area else area_list_from_config(cfg)):
        run_orders_for_area(cfg, a, date_key)
    log("orders.py 全部完成")
