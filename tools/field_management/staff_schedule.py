...
def sheet_names_for_date(date_key: str) -> tuple[str, str]:
    ...
    return f"專員班表查詢本月_{d.strftime('%m%d')}", f"專員班表查詢次月_{d.strftime('%m%d')}"


def run_staff_schedule_for_area(cfg: dict[str, Any], area: str, date_key: str) -> None:
    ...
    for file_date_key, target_sheet in [(date_key, current_sheet), (add_month_same_day_yyyymmdd(date_key, 1), next_sheet)]:
        ...
        values = ensure_rectangular([row[:7] for row in read_file_values(drive, sheets, file)], 7)
        if not values:
            raise RuntimeError(f"專員班表沒有資料：{file_base}")
        clear_and_write_values(sheets, target_spreadsheet_id, f"{target_sheet}!A:G", values)
        log(f"完成：{file.get('name')} → {target_sheet}!A:G / rows={len(values)}")
...
def main(date_key: str | None = None, area: str | None = None, system_name: str = "外場日排程系統") -> None:
    ...
    for a in ([area] if area else area_list_from_config(cfg)):
        run_staff_schedule_for_area(cfg, a, date_key)
