...
def render_html_table(
    ...
    split_payment_header: bool = False,
) -> None:
    if df.empty:
        st.info("尚無資料")
        return
    ...
    split_headers = []
    for col in show.columns:
        match = re.fullmatch(r"(\d{4}/\d{2})(待付款|已付款|待付款＋已付款)", str(col))
        split_headers.append(match.groups() if match else None)

    use_split_header = split_payment_header and any(split_headers)
    for col, parts in zip(show.columns, split_headers):
        if use_split_header and parts:
            out.append(f"<th>{html.escape(parts[0])}</th>")
        else:
            rowspan = ' rowspan="2"' if use_split_header else ""
            out.append(f"<th{rowspan}>{html.escape(str(col))}</th>")

    if use_split_header:
        out.append("</tr><tr>")
        for parts in split_headers:
            if parts:
                out.append(f"<th>{html.escape(parts[1])}</th>")
...
def render_report() -> None:
    ...
    with report_tabs[1]:
        ...
        st.markdown("#### 已付款")
        render_html_table(order_date_paid_df, split_payment_header=True)
        st.markdown("#### 待付款＋已付款")
        render_html_table(order_date_combined_df, split_payment_header=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
def queue_cetustek_import_and_allocate(*, month="", start_date=None, end_date=None, area="全區", qyear="", qmonth=""):
    if not area or area == "全區":
        raise ValueError("請先選擇區域")
    qyear = str(qyear or "").strip()
    qmonth = str(qmonth or "").strip()
    if not qyear or not qmonth:
        raise ValueError("請輸入鯨躍配號查詢年份與查詢月份")
    task = create_local_agent_task(
        "cetustek.import_and_allocate",
        {"area": area, "month": str(month or "").strip(), "qyear": qyear, "qmonth": qmonth, "cdp_url": "http://127.0.0.1:9222"},
        created_by=st.session_state.get("username", "Tool System"),
    )
    return f"任務已建立：{task['task_id']}（等待本機 Agent；會自動抓回 Drive 上的財政部字軌 CSV，依序匯入鯨躍並配號）"
...
FINANCE_TASKS = [
    ...
    {"name": "【鯨躍發票】電子發票字軌號碼匯入＋配號", "handler": queue_cetustek_import_and_allocate, "enabled": True},
...
with date_col:
    if system_type == "monthly_scheduler" and selected_function in monthly_order_functions:
        ...
        elif selected_function == "【鯨躍發票】電子發票字軌號碼匯入＋配號":
            st.markdown('<div class="field-label">📆 財政部字軌期別／鯨躍查詢年月</div>', unsafe_allow_html=True)
            _ia_start_month = today_date.month if today_date.month % 2 else today_date.month - 1
            _ia_next_start_month = _ia_start_month + 2
            _ia_next_year = today_date.year
            if _ia_next_start_month > 12:
                _ia_next_start_month = 1
                _ia_next_year += 1
            period = st.text_input(
                "財政部字軌期別",
                value=f"{_ia_next_year}{_ia_next_start_month:02d}",
                placeholder="例如：202609",
                label_visibility="collapsed",
                key="finance_cetustek_import_allocate_period",
            )
            _ia1, _ia2 = st.columns(2)
            with _ia1:
                cetustek_serial_qyear = st.text_input(
                    "鯨躍查詢年份", placeholder="下拉選單上的文字，例如 115", key="finance_cetustek_import_allocate_qyear"
                )
            with _ia2:
                cetustek_serial_qmonth = st.text_input(
                    "鯨躍查詢月份", placeholder="下拉選單上的文字，例如 09-10", key="finance_cetustek_import_allocate_qmonth"
                )
            st.caption(
                "財政部字軌期別格式 YYYYMM，用來從 Google Drive 抓回對應的財政部字軌 CSV；"
                "鯨躍查詢年月請填下拉選單實際顯示的文字。依序執行匯入→配號，任一步失敗就停止"
            )
            ...
            if selected_function == SERVICE_DEEP_CLEAN_SETTINGS:
                ...
                with st.expander("💰 加價與服務總額即時試算", expanded=False):
                    _preview_columns = [
                        ...
                        "P2 非VIP平日", "P2 VIP平日", "P2 非VIP週末", "P2 VIP週末",
                    ]
                    _preview_rates = [
                        deep_clean_phase1_nonvip_weekday_rate, deep_clean_phase1_weekday_rate,
                        deep_clean_phase1_nonvip_weekend_rate, deep_clean_phase1_weekend_rate,
                    ...
                    _extra_rows = [
                        ...
                        for hours in _preview_hours
                    ]
                    _total_rows = [
                        [f"2人{hours}小時", *[
                            hours * (base + rate)
                            for base, rate in zip(_preview_bases, _preview_rates)
                        ]]
                        for hours in _preview_hours
                    ]
                    st.write("年節加價")
...
if system_type == "finance_management" and selected_function == "【鯨躍發票】開立折讓單":
    try:
        ...
        _allowance_editor = st.data_editor(
            pd.DataFrame([
                {
                    "執行": False,
                    "列數": item["sheet_row"],
                    ...
                    "訂單姓名": item["customer"],
                    "原因": item["reason"],
                    "發票號碼": item["invoice_no"],
                    "含稅金額": item["refund_amount"],
                    "折讓未稅價": item["untaxed_amount"],
                }
                for item in _allowance_candidates
            ]),
            hide_index=True,
            use_container_width=True,
            disabled=["列數", "訂單編號", "訂單姓名", "原因", "發票號碼", "含稅金額", "折讓未稅價"],
            column_config={"執行": st.column_config.CheckboxColumn("執行")},
            key=f"cetustek_allowance_editor_{selected_area_value}",
        )
