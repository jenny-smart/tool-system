"""
檸檬家事 客服排程系統
tools/service_management/lemon_schedule_sync.py
...
執行方式：
  python -u -m tools.service_management.lemon_schedule_sync --step 0   # 全部
  python -u -m tools.service_management.lemon_schedule_sync --step 1
  python -u -m tools.service_management.lemon_schedule_sync --step 2
  python -u -m tools.service_management.lemon_schedule_sync --step 3
...
CONFIG: dict[str, Any] = {
    ...
    # 打卡 - 主控表
    "master_log_spreadsheet_id": os.environ.get(
...
def checkin_master(
    ...
    """打卡到主控表（TOOLS_APP_LOG_SPREADSHEET_ID）。"""
    master_id = CONFIG["master_log_spreadsheet_id"]
    if not master_id:
        return
    try:
        ss = gc.open_by_key(master_id)
        sh = _ensure_log_sheet(ss, CONFIG["master_log_sheet_name"])
        sh.append_row(
            [
                run_id,
...
def checkin_job(
    ...
    """打卡到執行檔（LEMON_TARGET_FILE_ID）。"""
    target_id = CONFIG["target_file_id"]
    if not target_id:
        return
    try:
